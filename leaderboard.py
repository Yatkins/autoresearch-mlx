...
#   invoices  report  description
# `score` = extractions-equal (PRIMARY, global per-cell); score_invoice = invoices-equal.
# `description` = what changed that run (from EXP_NOTE). Per-field accuracy is in the report.
# Ranking uses `score`. eval_set is parsed from the description prefix "[train]"/"[test]".
    ...
    for line in lines[1:]:  # skip header
        ...
        try:
            rows.append({
                ...
                "report": c[9], "desc": desc, "set": eval_set,
            })
        except ValueError:
            continue
    return rows
